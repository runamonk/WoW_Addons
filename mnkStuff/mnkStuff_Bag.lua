...
function mnkStuff_Bag.AddPocketTab(pocket, idx)
    ...
    pocket.button = CreateFrame('Button', 'button'..pocket.name, backpack, BackdropTemplateMixin and "BackdropTemplate")
    ...
    pocket.button.font = mnkLibs.createFontString(pocket.button, mnkLibs.Fonts.ap, 16, nil, nil, true)
...
function mnkStuff_Bag.AddItemToBackpack(itemlink, itemcount, bagnumber, slotnumber)
    ...
    if itemName == nil then
        local _, speciesID, _, itemRarity = (':'):split(itemlink)
    ...
    if i == -1 then
        ...
        pocket.stuff[i].itemicon.itemleveltext = mnkLibs.createFontString(pocket.stuff[i].itemicon, mnkLibs.Fonts.ap, 16, nil, nil, true)
        ...
        pocket.stuff[i].itemicon.itemleveltext:SetText(mnkLibs.Color(COLOR_YELLOW) .. 'i'..(GetDetailedItemLevelInfo(itemlink) or "1"))
        
        pocket.stuff[i].itemicon.itemtext = mnkLibs.createFontString(pocket.stuff[i].itemicon, mnkLibs.Fonts.ap, 16, nil, nil, true)
        ...
        pocket.stuff[i].itemicon.itemcountext = mnkLibs.createFontString(pocket.stuff[i].itemicon, mnkLibs.Fonts.ap, 16, nil, nil, true)
...
function mnkStuff_Bag.CreateBackpack()
    ...
    backpack = CreateFrame('Frame', 'backpack', UIParent, BackdropTemplateMixin and "BackdropTemplate")
    backpack.pockets = {}
    ...
    backpack.slider = CreateFrame('Slider', nil, backpack.scrollframe, BackdropTemplateMixin and "BackdropTemplate")
    ...
    backpack.content = CreateFrame('Frame', nil, backpack.scrollframe, BackdropTemplateMixin and "BackdropTemplate")
    ...
    function mnkStuff_Bag.CreateBackpackBags()
        ...
        backpack.bagsframe = CreateFrame('Frame', 'backpack.bagsframe', backpack, BackdropTemplateMixin and "BackdropTemplate")
        ...
        for b = 1, NUM_BAG_SLOTS do
            ...
            backpack.bagsframe.bags[b].button = CreateFrame('Button', 'bag'..b, backpack.bagsframe, BackdropTemplateMixin and "BackdropTemplate")
    ...
    function mnkStuff_Bag.GetPocket(itemlink)
        ...
        if itemRarity == LE_ITEM_QUALITY_POOR then
            pocketname = 'Junk'
        elseif string.find(itemName, 'Hearthstone') or string.find(itemName, 'Flight Master') then
    ...
    function mnkStuff_Bag.OpenPocket(pocket)
        ...
        for b = 1, #pocket.stuff do
            ...
            if pocket.stuff[b].itemCount > 1 then
                pocket.stuff[b].itemicon.itemcountext:SetText(mnkLibs.Color(COLOR_RED) .. ' × '..mnkLibs.Color(COLOR_GOLD)..pocket.stuff[b].itemCount)
            else
