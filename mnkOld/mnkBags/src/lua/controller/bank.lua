...
function mnkBagsBankBar_OnShow(self)
    BankFrame:Show()
    BankFrame.selectedTab = 1
    PanelTemplates_SetTab(self, 1)
    mnkBagsBankContainer:Show()
    mnkBagsReagentContainer:Hide()
    self:Update()
    controller:Register()
    controller:Update()
...
function mnkBagsBankBar_OnHide(self)
    BankFrame:Hide()
...
function mnkBagsBankTab_OnClick(tab)
    PanelTemplates_SetTab(mnkBagsBankBar, tab.tab)
    if tab.tab == 1 then
        mnkBagsBankContainer:Show()
        mnkBagsReagentContainer:Hide()
        BankFrame.selectedTab = 1
    else
        mnkBagsBankContainer:Hide()
        mnkBagsReagentContainer:Show()
        BankFrame.selectedTab = 2
    end
end

function controller:DepositReagents()
    if ADDON.settings:GetSettings(mnkBags_TYPE_MAIN)[mnkBags_SETTING_DEPOSIT_REAGENT] then
        DepositReagentBank();
    end
end
...
function controller:Update()
    ...
    mnkBagsBankContainer:Arrange()
    self:ArrangeReagents()
end

function controller:OnItemsCleared()
    if mnkBagsBankBar:IsVisible() then
        self:Update()
    end
end
...
function controller:ArrangeReagents()
    if IsReagentBankUnlocked() then
        mnkBagsReagentContainer.reagentButton:Hide()
        mnkBagsReagentContainer:Arrange()
    else
        mnkBagsReagentContainer:SetSize(110, 35)
        mnkBagsReagentContainer.reagentButton:Show()
    end
end
...
function controller:ITEM_LOCK_CHANGED(bag, slot)
    ...
    if bag == BANK_CONTAINER and slot > NUM_BANKGENERIC_SLOTS then
        _G[mnkBagsBankBar:GetName() .. 'Bag' .. slot-NUM_BANKGENERIC_SLOTS]:UpdateLock()
    else
