...
function ADDON:NewItemContainer(name, parent)
    local frame = CreateFrame('Frame', 'mnkBagsItemContainer_' .. name, parent or UIParent, 'mnkBagsItemContainerTemplate')
...
function container:Init(name)
    self.__type = mnkBags_TYPE_ITEM_CONTAINER
...
function container:UpdateFromSettings()
    ...
    self:SetTitleSize(settings[mnkBags_SETTING_TEXT_SIZE])
    self:SetColors(
        settings[mnkBags_SETTING_BACKGROUND_COLOR],
        settings[mnkBags_SETTING_BORDER_COLOR]
    )
    self:SetTextColor(settings[mnkBags_SETTING_TEXT_COLOR])
