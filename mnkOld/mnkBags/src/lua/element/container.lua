...
function mnkBagsContainerLoad(self)
    ADDON:CreateAddon(self, container)
end

function container:Init()
    self.__type = mnkBags_TYPE_CONTAINER
...
function container:UpdateFromSettings()
    ...
    self:SetPadding(settings[mnkBags_SETTING_PADDING])
    self:SetSpacing(settings[mnkBags_SETTING_SPACING])
end
...
function container:Arrange(override)
    ADDON.format[ADDON.settings:GetSettings(self.__type)[mnkBags_SETTING_FORMATTER]](
        self,
        ADDON.settings:GetSettings(self.__type)[mnkBags_SETTING_FORMATTER_MAX_ITEMS],
        ADDON.settings:GetSettings(self.__type)[mnkBags_SETTING_FORMATTER_MAX_HEIGHT],
        ADDON.settings:GetSettings(self.__type)[mnkBags_SETTING_FORMATTER_BOX_COLS],
        ADDON.settings:GetSettings(self.__type)[mnkBags_SETTING_FORMATTER_VERT],
        override == 2
    )
end