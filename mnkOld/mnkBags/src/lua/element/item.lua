...
function ADDON:NewItem(bag, slot)
    local frame = CreateFrame('Frame', string.format('mnkBagsItem_%d_%d', bag, slot))
    frame.button = CreateFrame('Button', string.format('mnkBagsItemButton_%d_%d', bag, slot), frame,
...
function item:OnClick(button)
    if self:GetParent().id and IsAltKeyDown() and button == 'LeftButton' then
            mnkBagsCategoryDialogLoad(self:GetParent().id, self:GetParent().name)
    end
end
...
function item:GetContainerName()
    if self.id then
        ...
        --print(self.name.." "..self.class.." "..self.subClass.." "..self.quality);
        ...
        local subClassSplitList = ADDON.settings:GetSettings(mnkBags_TYPE_SUB_CLASS)
        if self.classId ~= LE_ITEM_CLASS_BATTLEPET then
            if subClassSplitList[mnkBags_SETTING_BOE] and mnkBagsTooltip:IsItemBOE(self.link) then
                return mnkBags_LOCALE_BOE
            end
            if subClassSplitList[mnkBags_SETTING_BOA] and mnkBagsTooltip:IsItemBOA(self.link) then
                return mnkBags_LOCALE_BOA
            end
        end

        if mnkBagsTooltip:IsItemArtifactPower(self.link) then
            return "Artifact Power"
        end
        
        if mnkBagsTooltip:IsItemChampionEquip(self.link) then
            return "Champion Equipment"
        end
  
        if mnkBagsTooltip:IsItemArmorToken(self.link) then
            return "Armor"
        end      
...
function item:Update()
    ...
    if isEquipment then
        level = mnkBagsTooltip:GetItemLevel(link) or level
