...
function mnkMoney.DoOnEnter(self)
    ...
    if gold > 0 then
        tooltip:AddLine(format("|T%s:16|t %s", "Interface\\MoneyFrame\\UI-GoldIcon", "Gold"), SPACER, format("%d", gold)); 
    elseif silver > 0 then
    ...
    for i = 1, #t do
        tooltip:AddLine(t[i].icon..t[i].name, SPACER, t[i].count); 
    end
