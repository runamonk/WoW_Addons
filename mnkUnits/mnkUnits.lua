...
local function CreateCastBar(self)
    ...
    self.castbarbg:SetSize(self:GetWidth()+1, 18)
...
local function CreateBottomPanel()
    ...
    pplayer:SetWidth(279)
	pplayer:SetPoint('CENTER', UIParent, 'BOTTOM', 0, 202)
    ...
    mnkLibs.createBorder(pbackRight, 0, -0.8, 0, 0, {classColor.r, classColor.g, classColor.b, .5})
    pbackRight:Show()  
end
...
local function PlayerUnit(self)
    if Config.showplayer then
        ...
        self.HealthValue:SetPoint('LEFT', self.Health, 2, 0)
...
local function UpdateMirrorBars()
    for i = 1, 3 do
        ...
        bar.border:Hide()
        mnkLibs.createBorder(bar, 0.8, -0.8, -0.8, 0.8, {1,1,1,1})
    end
end
