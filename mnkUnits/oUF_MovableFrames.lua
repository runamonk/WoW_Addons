    ...
    opt:SetScript("OnShow", function(self)
        ...
        do
            ...
            local OnChar = function(self, key)
                ...
                (self.onlyAboveZero and key == '-' and not (self:GetNumber() < 0))) then
        ...
        if(not _LOCK) then
            for k, obj in next, oUF.objects do
                if(not obj.disableMovement) then
                    local style, identifier, isHeader = getObjectInformation(obj)
                    -- When oUF are used to create nameplates
                    -- if you have unit targetted and a nameplate is showing
                    -- it will get picked up here and fill the screen with
                    -- a backdrop
                    if not string.find(identifier, style.."NamePlate") then 
